        ...
        try:
            ...
            if results.multi_face_landmarks:
                ...
                for id, faceLms in enumerate(results.multi_face_landmarks):
                    ...
                    mouthPositionTop = (
                        int(faceLms.landmark[306].x * iw), int(faceLms.landmark[306].y * ih))
                    mouthPositionBottom = (
                        int(faceLms.landmark[61].x * iw), int(faceLms.landmark[61].y * ih))
                    ...
                    rightEyeRegion = np.array([
                        ...
                        (faceLms.landmark[246].x * iw,
                        faceLms.landmark[246].y * ih),
                        (faceLms.landmark[161].x * iw,
                        faceLms.landmark[161].y * ih),
                        (faceLms.landmark[160].x * iw,
                        faceLms.landmark[160].y * ih),
                        (faceLms.landmark[159].x * iw,
                        faceLms.landmark[159].y * ih),
                        (faceLms.landmark[158].x * iw,
                        faceLms.landmark[158].y * ih),
                        (faceLms.landmark[157].x * iw,
                        faceLms.landmark[157].y * ih),
                        (faceLms.landmark[173].x * iw,
                        faceLms.landmark[173].y * ih),
                        (faceLms.landmark[133].x * iw,
                        faceLms.landmark[133].y * ih),
                        (faceLms.landmark[155].x * iw,
                        faceLms.landmark[155].y * ih),
                        (faceLms.landmark[154].x * iw,
                        faceLms.landmark[154].y * ih),
                        (faceLms.landmark[153].x * iw,
                        faceLms.landmark[153].y * ih),
                        (faceLms.landmark[145].x * iw,
                        faceLms.landmark[145].y * ih),
                        (faceLms.landmark[144].x * iw,
                        faceLms.landmark[144].y * ih),
                        (faceLms.landmark[163].x * iw,
                        faceLms.landmark[163].y * ih),
                        (faceLms.landmark[7].x * iw, faceLms.landmark[7].y * ih)], np.int32)

                    leftEyeRegion = np.array([
                        (int(faceLms.landmark[398].x * iw),
                        int(faceLms.landmark[398].y * ih)),
                        (int(faceLms.landmark[384].x * iw),
                        int(faceLms.landmark[384].y * ih)),
                        (int(faceLms.landmark[385].x * iw),
                        int(faceLms.landmark[385].y * ih)),
                        (int(faceLms.landmark[386].x * iw),
                        int(faceLms.landmark[386].y * ih)),
                        (int(faceLms.landmark[387].x * iw),
                        int(faceLms.landmark[387].y * ih)),
                        (int(faceLms.landmark[388].x * iw),
                        int(faceLms.landmark[388].y * ih)),
                        (int(faceLms.landmark[466].x * iw),
                        int(faceLms.landmark[466].y * ih)),
                        (int(faceLms.landmark[263].x * iw),
                        int(faceLms.landmark[263].y * ih)),
                        (int(faceLms.landmark[249].x * iw),
                        int(faceLms.landmark[249].y * ih)),
                        (int(faceLms.landmark[390].x * iw),
                        int(faceLms.landmark[390].y * ih)),
                        (int(faceLms.landmark[373].x * iw),
                        int(faceLms.landmark[373].y * ih)),
                        (int(faceLms.landmark[374].x * iw),
                        int(faceLms.landmark[374].y * ih)),
                        (int(faceLms.landmark[380].x * iw),
                        int(faceLms.landmark[380].y * ih)),
                        (int(faceLms.landmark[381].x * iw),
                        int(faceLms.landmark[381].y * ih)),
                        (int(faceLms.landmark[382].x * iw),
                        int(faceLms.landmark[382].y * ih)),
                        (int(faceLms.landmark[362].x * iw),
                        int(faceLms.landmark[362].y * ih))
                    ], np.int32)
                    ...
                    leftEyeFrame = img[leftEyeMinY: leftEyeMaxY,
                                    leftEyeMinX: leftEyeMaxX]
                    ...
                    leftEyeGray = leftEyeMasked[leftEyeMinY: leftEyeMaxY,
                                                leftEyeMinX: leftEyeMaxX]

                    #   DETECTING THE POSITION OF THE IRIS (LEFT)
                    _, thresholdEye = cv2.threshold(
                        leftEyeGray, 70, 255, cv2.THRESH_BINARY)
                    ...
                    rightEyeGray = rightEyeMasked[rightEyeMinY: rightEyeMaxY,
                                                rightEyeMinX: rightEyeMaxX]

                    # DETECTING THE POSITION OF THE IRIS (RIGHT)
                    _, thresholdEyeRight = cv2.threshold(
                        rightEyeGray, 70, 255, cv2.THRESH_BINARY)
                    heightRight, widthRight = thresholdEyeRight.shape
                    leftSideThresholdRight = thresholdEyeRight[0:heightRight, 0:int(
                        widthRight / 2)]
                    leftSideWhiteRight = cv2.countNonZero(leftSideThresholdRight)
                    rightSideThresholdRight = thresholdEyeRight[0:heightRight, int(
                        widthRight / 2):]
